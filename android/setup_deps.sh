...
# SDL 2.32.x, not 2.28.5: the older release calls ALooper_pollAll, which the
# NDK 27+ headers mark as unavailable, and only the newer releases ship the
# 16 KB page-size support Android 15+ requires.
[ -d SDL2 ]       || git clone --depth=1 --branch release-2.32.8 https://github.com/libsdl-org/SDL.git SDL2
